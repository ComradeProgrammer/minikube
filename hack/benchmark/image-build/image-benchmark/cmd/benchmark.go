...
func main() {
	...
	images := flag.String("images", "", "a list of images we use, seperated by comma")

	iters := flag.String("iters", "iterative,non-iterative", "a list of flows we use, seperated by comma")

	benchMethods := flag.String("bench-methods", "", "a list of benchmark method names we use,seperated by comma")
	...
	results, err := benchmark.Run(*runs, benchmark.NewBenchMarkRunConfig(*profile, *images, *iters, *benchMethods, extraMinikubeStartArgs))
	if err != nil {
